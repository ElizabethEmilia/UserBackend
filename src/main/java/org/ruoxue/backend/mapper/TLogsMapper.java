...
//    查询日志的列表
    ...
    @Select("(select t_logs.*, t_admin.name from t_logs ,t_admin where t_logs.aid=t_admin.id ) union (select t_logs.*, '系统' as name from t_logs where t_logs.aid=-1 )")
    List<Map<String, Object>> countListLogs();
