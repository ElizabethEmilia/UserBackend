...
//    当月专票金额
    ...
    // 当月所有票应缴纳税金之和
    @Select("SELECT sum(pretax) FROM t_receipt WHERE DATE_FORMAT( tm_submit, '%Y%m' ) = DATE_FORMAT( CURDATE( ) , '%Y%m' ) and cid=#{cid} and status=2")
    Double countPreTaxSummationMonthly(@Param("cid") Integer cid);

    // 当年所有票应缴纳税金之和
    @Select("SELECT sum(pretax) FROM t_receipt WHERE YEAR(tm_submit)=YEAR(NOW()) and cid=#{cid} and status=2")
