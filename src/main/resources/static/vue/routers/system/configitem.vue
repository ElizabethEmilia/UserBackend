    ...
    <div class="margin-top: 10px;">
        <div class="inline item">
            ...
            <div  class="inline input">
                <InputNumber @on-focus="recordOldValue" @on-blur="update" v-if="type == 'int'" v-model="inputValue" placeholder="整数值" :disabled="disabled"></InputNumber>
                <InputNumber @on-focus="recordOldValue" @on-blur="update" v-else-if="type == 'number'" v-model="inputValue" placeholder="数字" :disabled="disabled"/>
                <Input @on-focus="recordOldValue" @on-blur="update" v-else-if="type == 'string'" v-model="inputValue" placeholder="字符串值" :disabled="disabled"/>
                <Input @on-focus="recordOldValue" @on-blur="update" type="textarea" v-else-if="type == 'text'" v-model="inputValue" placeholder="文本"  :rows="4" :disabled="disabled"/>
                <DatePicker @on-change="update" type="date" v-else-if="type == 'date'" v-model="inputValue" placeholder="选择日期" :disabled="disabled"></DatePicker>
                <Checkbox @on-change="update" v-else-if="type == 'bool'" v-model="inputValue" :disabled="disabled"></Checkbox>
                <Select @on-change="update" v-else-if="type == 'enum'" v-model="inputValue.value" placeholder="选择一项" :disabled="disabled">
                    <Option v-for="(e, i) in inputValue.options" :key="i" :value="i">{{e}}</Option>
                </Select>
                <Select @on-change="update" multiple v-else-if="type == 'senum'" v-model="inputValue.value" placeholder="选择一项或多项" :disabled="disabled">
                    <Option v-for="(e, i) in inputValue.options" :key="i" :value="i">{{e}}</Option>
                </Select>
                <InputNumber  @on-focus="recordOldValue" @on-blur="update" v-else-if="type == 'uint'" v-model="inputValue" placeholder="无符号整数" :disabled="disabled"/>
                <Input @on-focus="recordOldValue" @on-blur="update" type="password" v-else-if="type == 'password'" v-model="inputValue" :placeholder="value == ''?'密码':'安全原因，密码未予显示'" :disabled="disabled"/>
...
export default {
    ...
    methods: {
        ...
        update() {
            if (this.disabled || !this.valueChanged)
                return;
            let h = this.handlers[this.type];
            if (typeof h == "undefined") return;
            if (!h.validate(this.inputValue)) {
                this.inputValue = h.defaultValue;
                this.typeErr = '设置值未通过格式校验';
                return;
            }
            this.updateValue(h.stringify(this.inputValue));
        },
        recordOldValue() {
